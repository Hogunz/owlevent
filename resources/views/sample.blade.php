<x-guest-layout>

    <!-- sample 1 -->
    <section class="p-8 flex justify-center items-center h-screen">
        <div class="relative">
            <img src="images/try2.png"
                class="max-w-lg border bg-gradient-to-b from-gray-300 to-black p-8 rounded-lg rotate-12 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10"
                alt="..." />

            <img src="images/try2.png"
                class="max-w-lg border bg-gradient-to-b from-gray-300 to-black p-8 rounded-lg absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20"
                alt="..." />
        </div>
    </section>


    <!-- sample 2 -->
    <section class=" ">
        <div class="relative bg-gray-600  w-full" style="height:75vh;">
            <div class="absolute bottom-0 right-0">
                <img class="lg:max-w-6xl sm:max-w-lg lg:w-full" src="images/try2.png" alt="" data-aos="zoom-out"
                    data-aos-duration="1000">
            </div>
            <div class="absolute bottom-0 left-0">
                <img class="lg:max-w-6xl sm:max-w-lg lg:w-full" src="images/try2.png" alt="" data-aos="zoom-out"
                    data-aos-duration="1000">
            </div>
        </div>
    </section>


    <!-- sample 3 -->

    <!-- sample 3 -->
    <section class="p-8 flex justify-center items-center h-screen">
        <div class="relative">
            <div
                class="max-w-lg border bg-gradient-to-b from-blue-300 to-red-300 p-8 rounded-lg rotate-12 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10">
            </div>
            <img src="images/try2.png"
                class="max-w-lg border bg-gradient-to-b from-gray-300 to-black p-8 rounded-lg absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50"
                alt="..." />
        </div>
    </section>




</x-guest-layout>
